...
async def test_ab_all_orthogonal_covers_no_change() -> None:
    ...
    key_map = {
        _mod.image_embedding_cache_key_for_url(
            f"https://i0.hdslb.com/bfs/archive/{s.bvid}.jpg"
...
async def test_ab_aligned_covers_flip_top_k() -> None:
    ...
    align = CandidateSpec(
        bvid="BV1ALIGN", relevance_score=0.80, target_cos=0.5, topic_group="topic_0"
    )
    plain = CandidateSpec(
        bvid="BV1PLAIN", relevance_score=0.83, target_cos=0.0, topic_group="topic_1"
    )
    specs = [align, plain]
    contents = [_spec_to_content(s) for s in specs]
    anchor = _unit_anchor(64)
    key_map = {
        _mod.image_embedding_cache_key_for_url(
            "https://i0.hdslb.com/bfs/archive/BV1ALIGN.jpg"
        ): _cover_vec_for_cosine(0.5, 64),
        _mod.image_embedding_cache_key_for_url(
            "https://i0.hdslb.com/bfs/archive/BV1PLAIN.jpg"
        ): _cover_vec_for_cosine(0.0, 64),
    }
    emb = _SyntheticEmb(anchor, key_map)
    bonus = _compute_bonus_map(specs, contents, emb)
...
async def test_ab_bonus_matches_real_formula() -> None:
    ...
    specs = [
        CandidateSpec(bvid="BVX", relevance_score=0.80, target_cos=0.40, topic_group="topic_0")
    ]
    contents = [_spec_to_content(s) for s in specs]
    anchor = _unit_anchor(64)
    ...
    key_map = {
        _mod.image_embedding_cache_key_for_url(
            "https://i0.hdslb.com/bfs/archive/BVX.jpg"
        ): cover_vec
    }
    emb = _SyntheticEmb(anchor, key_map)
    bonus = _compute_bonus_map(specs, contents, emb)
