...
📌 最新版本：**v0.3.223（2026-09-17）**

- **网页「加载更多 / 换一批」不再固定 403**：反向代理 / 外部 TLS 终结部署下推荐接口保留原始 Host，CSRF 同源判定恢复一致。
- **Windows 桌面版启动与安装更稳（issue #250）**：LLM 配置暂不可用时后台进程降级等待而非弹窗退出、崩溃自动重启；卸载运行中的应用会被标准对话框拦截，交互安装点「完成」后才启动。
- **推荐理由不再把旧内容说成「最新」**：生成时携带内容发布时间与该条评估时刻，禁止靠标题年份或模型知识猜测时效。
- **移动端原生播放页新增 UP 主信息与关注能力**：后端透传 UP 主卡片与关注 / 取关接口，Cookie 与 CSRF 令牌仍只留在后端。
