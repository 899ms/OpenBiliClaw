...
- **文档同步**：`docs/modules/api.md`（approve 异步响应协议 + 前端迁移说明、agent stream 租约超时与兜底一致性、approvals 状态过滤新增 executing/failed）、`docs/modules/agent.md`（状态机、异步执行、工作纪律、兜底一致性）、`docs/modules/runtime.md`（cancel_all 豁免名单 + store 复用）、`docs/modules/memory.md`（tmp 唯一后缀）。
