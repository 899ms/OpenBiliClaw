...
| 后台 worker 降级等待与存活看护（issue #250） | ✅ | 四进程桌面版在配置无法构造任何 LLM 实例时不再让子进程崩溃弹窗：`worker.main.run_full_worker()` 用 `build_llm_registry(load_config())` 纯构造探针（无网络）每 15s 重读 `config.toml` 重试，等待期心跳继续写 `worker_status.json`，成功后才构建完整 runtime；`discovery_worker.run_discovery_worker()` 同样探针门 + 重试（失败尝试的 degraded 数据库会先关闭），拿不到 `runtime_controller.run_forever` 只记日志，不再抛未捕获 `RuntimeError`。`recommendation_server.wait_for_buildable_llm()` 在 `create_app()` 前同样等待可构造配置，主 API 恢复后推荐进程自动开始服务（不再永久 degraded）。`packaging/entry.py` 的子进程分发捕获异常并写 `desktop.log` 后 `SystemExit(1)`，窗口化构建因此不会再弹 PyInstaller 对话框；父进程用 `proc.ChildProcessSupervisor` 每 5s 巡检后端子进程，崩溃后按 2/4/8/...（上限 60s）退避重启，子进程存活 ≥5 分钟重置退避，退出时逆序 terminate→kill。API 侧 `RuntimeContext.full_worker_expected()` / `warn_if_full_worker_heartbeat_stale()` 在已委托但心跳文件存在且过期时打 WARNING（启动窗口无心跳文件不误报）；`/api/runtime-status` 继续用 `worker_running` / `worker_heartbeat_age_seconds` 暴露该降级状态。回归测试见 `tests/test_worker_degraded_boot.py` 与 `tests/test_proc_supervisor.py`。 |
...
| 桌面安装包升级进程交接 | ✅ | Windows Inno Setup 在覆盖文件前通过 Restart Manager + `taskkill /T /F` 结束旧版进程树；安装成功后的启动按安装模式分流：交互安装由 Finish 页默认勾选的 `postinstall` 复选框（`postinstall nowait skipifsilent`）接管——点「完成」才启动、可取消勾选，向导未结束时绝不抢跑；`/SILENT` / `/VERYSILENT` 下 `postinstall` 条目由 `skipifsilent` 跳过（静默安装会模拟点过隐藏的 Finish 页复选框，单凭「没有 Finish 页」并不足以拦住它），由 `nowait skipifnotsilent` 条目自动从 `{app}\OpenBiliClaw.exe` 拉起新版本，保证静默升级在杀掉旧进程后必有新进程交接；两条条目必须靠各自的 skip 标志保持互斥。`tests/test_installer_script.py` 锁定该 [Run] 契约（恰好一条 postinstall + 一条 silent-only，禁止无条件条目），windows-latest CI 的 `/VERYSILENT` 安装步骤覆盖静默交接：以「安装器拉起的实例写出 profile 标记 `config.toml`」为断言，不再是空管道 no-op（selftest 实例装配完即退出，查存活进程会竞态）。macOS DMG 因系统模型没有安装完成回调，根目录提供显式 `安装并启动 Install OpenBiliClaw.command`：先用 `ditto` 完整暂存并校验 bundle version + code signature，再发起有界的优雅退出请求（连 `osascript` 等待本身也受窗口约束；超时 TERM/KILL，且只清理由旧 OpenBiliClaw.app 拉起的内置 Ollama）、同卷备份后原子替换 `/Applications/OpenBiliClaw.app`，二次校验失败或中断会恢复旧 app，成功后 `open -n` 精确启动安装路径并确认进程出现。助手不自动删除 quarantine；Gatekeeper 决策仍由 macOS 和用户完成。传统拖拽保持兼容，但需手动退出、替换、重开。macOS-only E2E 使用两个 ad-hoc 签名最小 app 和真实只读挂载 DMG，覆盖旧 PID 退出、版本/签名更新、新 PID 启动与暂存清理。 升级交接之外，卸载侧由两层保护：`packaging/entry.py` 的 `_acquire_installer_mutex()` 让每个冻结进程（托盘主进程与 `--openbiliclaw-worker` 子进程）全程持有会话级命名互斥体（`_INSTALLER_MUTEX_NAME`，不加 `Global\` 前缀以免跨用户误拦截，fail-open，进程退出自动释放），Inno `AppMutex` 使 Setup 与 Uninstall 启动即弹标准「应用正在运行」对话框并重检——Restart Manager 不适用于卸载，且卸载器对占用文件的非致命错误会自删残留；`[Code] CurUninstallStepChanged(usUninstall)` 复用 `StopRunningInstance` 的 `taskkill /T /F` 兜底——必须在 AppMutex 门禁**之后**执行（Inno 卸载器先跑 `[Code] InitializeUninstall` 事件再做内部互斥体检查，把强杀放进那里会先杀掉持锁进程、让门禁永远静默通过，真机验证过该反例），清理无互斥体的旧安装与孤儿 worker/ollama 子进程。真机验证：应用运行中触发卸载被拦（日志记录 "Defaulting to Cancel for suppressed message box: Uninstall has detected that OpenBiliClaw is currently running"），应用与文件零改动、可重试。 |
| 扩展原生保存共享 runtime | ✅（6/6 executor + 真实账号验证） | 扩展已定义与后端一致的 `native_save` task/result allow-list、canonical HTTPS URL 规则、`NATIVE_SAVE_EXECUTE` / `NATIVE_SAVE_RESULT` 消息契约和 active-tab runner，并统一经过共享 MV3 recovery barrier。一般 runner 与 legacy dispatcher 共用 `globalThis` mutex 保护 tab 创建/加载，加载完成即释放；XHS 手动 native-save 使用 exact tokenized route + identity/control fence，可在没有精确复用页时越过后台 discovery mutex，且 alarm/runtime wake poll single-flight。执行中的任务按 task/tab 独立分桶，仍各自在单一绝对 deadline 内严格校验 final tab 与 sender URL、tab/ID/item/platform。`chrome.storage.session` 可同时记录多个 runner-owned tab，MV3 recovery 只关闭这些 ID；content 用 256 项 bounded outcome-promise cache。YouTube duplicate exact playlist 优先 checked proof，否则稳定复用一个；知乎 typed `question/answer/article` identity 绑定当前全局 `收藏 / 已收藏` 控件，初始已收藏和 verification 均零点击；小红书适配 current `noteContainer/collect-wrapper`。 |
...
| 扩展在线账号信号周期回拉 | ✅ | `runtime.source_incremental_sync.SourceIncrementalSync.tick()` 是 `ContinuousRefreshController` 内独立的 60 秒决策 loop。`scheduler.source_incremental_enabled` 默认 `false`，且每个来源的 `sources.<slug>.incremental_enabled` 也默认 `false`；总开关关闭时在 presence 前返回，因而不会自动创建任务或触碰浏览器；旧配置缺字段也按关闭处理。显式开启总开关后，仍只对 `incremental_enabled=true` 的来源继续要求 `scheduler.enabled`、完整画像、guided init 非活跃和 `PresenceTracker.is_present()`，再按持久 round-robin 在 XHS / 抖音 / YouTube / 知乎 / Reddit / Linux.do / V2EX 复用现有 bootstrap scope；全局周期默认 24 小时，抖音逐源仍默认 `0`。七表 `pending/in_progress` 是全局 active 权威，进程内 decision lock 覆盖热重载旧 `to_thread`，SQLite `BEGIN IMMEDIATE` admission transaction 则把七表扫描与 insert 原子串行到跨 facade / 进程。崩溃收养按任务 `created_at` 补齐 attempt/cursor；只有新建且非空 task id 才写 `last_attempt_at/cursor/active_task` 并经当前 `EventHub` 唤醒扩展。预算耗尽会在同 tick 安全尝试下一到期源，异常/复用不推进时间；只有 evidence-backed `ok/empty` 完整终态写 `last_success_at` 并推进 cadence，failed/degraded 清理 active 后立即可重试。guided init 成功会同时种下首次 attempt/success，避免初始化后立刻重复拉取；失败不种。手动 init / fetch 和 discovery 不受该开关影响。GitHub 明确不加入扩展任务或该周期回拉。已停用来源（discovery 或周期回拉开关关闭）的 `pending/in_progress` 任务不会占用全局 active 槽，避免卡住其它来源；scheduler-owned 增量任务会在对应 `/next-task` 领取前被清理，手动任务保持 `pending` 等待重新启用。 |
...
| GitHub 后台 discovery producer | 🧪 接线；验收见 ledger | `GitHubDiscoveryProducer.produce_if_due()` 在 GitHub 平台族低于 quota 且启用时，只执行当前 `source_modes` 选择的公开 repository `search / ranked / latest`。CLI、正式 producer 与 inspiration backend 共用 public query sanitizer，强制 `is:public`，normalizer 再拒绝 `private=true`；可选 PAT 只用于提额和身份校验，401 后按指纹记录拒绝并退回匿名公开发现。producer 复用 canonical 关键词 claim/use/rollback，按最终去重且真正 enqueue 的候选扣 UTC 日预算，持久化 page cursor/run ledger。来源级 `Retry-After` cooldown 由 formal 与 inspiration 路径共用，任一路径限流都会让另一条 fail-closed 跳过；本地 status 也只聚合当前启用 modes，关闭分支不制造 `unverified`。它只把 `content_type="repository"` raw candidates 交给共享 evaluator，没有插件任务、Cookie、后台账号增量、媒体抓取或任何 GitHub 写操作。本行只记录实现边界；真实 API / E2E 证据状态以 [`platform-source-acceptance.github.md`](../platform-source-acceptance.github.md) 为准。 |
...
| 惊喜永久消费 | ✅ | `mark_delight_sent()` 只表示通知已送达并维护冷却，不置 `delight_seen`，因此已推送但未看过的候选仍可被 popup 重灌；`mark_delight_seen()` 供用户主动叉掉时委托 storage 写 canonical `seen_items` 并置 `delight_seen`，随后更新主动惊喜冷却。两种动作分开，避免“通知出现过”被误当成“用户看过”。 |
...
| Activity feed 状态摘要 | ✅ | `/api/activity-feed` 在线程中聚合认知更新、反馈、推荐池补货和 live summary，并通过单入口复用缓存，避免阻塞换批代理；未初始化且还没有推荐 / 可换池 / 补货产物时，普通 `/api/events` 不会新写入 pending signals，旧的 `pending_signal_events` 也不会抢占初始化提示。初始化后 pending 文案统一为“已记下 N 个新动作，下一轮补货会拿来参考”，表示 discovery refresh 水位，不表示画像待处理队列。 |
...
| 多源 bootstrap 去重 | ✅ | `/api/sources/{xhs,dy,yt,zhihu,reddit,linuxdo,v2ex,weibo}/task-result` 会用原子的 `source_bootstrap_state.json` 过滤跨任务旧 identity key；每源按响应顺序保留最新 5,000 个，任务首份 canonical 结果仍完整保留，只有新增项进入 durable event / profile owner 路径。 | GitHub 不进入扩展 task-result staging。
| 扩展任务 claim / 复用 | ✅ | XHS / 抖音 / YouTube / 知乎 / Reddit / Linux.do / V2EX / 微博 bootstrap 任务在扩展 poll 时用短生命周期 SQLite 连接标记 `in_progress`，CLI 默认复用 6 小时内近期可复用任务，避免重复打开任务 tab 全量扫描。Linux.do 的 `failed/degraded` 明确不参与复用：已取到的 degraded 事件保留，下次仍新建任务补齐失败 scope。周期调度、CLI fetch 与 guided init 共用 enqueue decision core：进程锁防线程交叠，SQLite `BEGIN IMMEDIATE` 把八表全局 active scan 与选中来源 insert 置于同一事务；`force` 只绕过近期终态复用，不能制造并行账号任务。 |
...
扩展共享原生保存基础（6/6 executor 已接、fixture 全覆盖；当前知乎目标为全局 `知乎收藏`，不声明命名收藏夹）：
...
XHS / 抖音 / YouTube / 知乎 / Reddit / Linux.do / V2EX / 微博八个插件任务桥保留两层去重；GitHub 不创建扩展任务，因此不进入这套 task-result staging：
...
- 跨任务：API 在传播 bootstrap 事件前读取 `source_bootstrap_state.json`，跳过八个平台已经进入 durable event ingress 的 identity key；seen-key checkpoint 使用原子 read-modify-write，按扩展响应顺序为每源保留最新 5,000 个。这样 `fetch-*`、`init`、周期回拉或近期任务复用重复返回同一批收藏 / 历史时，不会再次插入行为事实或触发画像更新。各平台 final callback 先冻结首份 canonical result，随后依次修复 event ingress、seen state 和 terminal flip；任一步失败都保持 staged/nonterminal，供租约重领后从冻结结果重放。Linux.do scope 严格映射为 bookmarks→favorite、likes→like、read_history→view；V2EX key 额外带 resolved username 前缀，后端净化任务字段、执行 identity mismatch gate，并对账号分区 Node evidence / 收藏 snapshot effect 做幂等记录；微博 key 按已观察账号分区。
