...
- **ToneProfile** — 从画像、偏好和近期反馈推断语气风格，用于推荐、画像总结和对话；`[soul] reply_style`（issue #255）可在语气块追加一行自由文本指令，为空时 prompt 逐字节不变
...
20. **用户自定义语气只追加、不改写**：`[soul] reply_style`（issue #255）非空时在四类用户向 prompt 的语气块末尾追加一行 `- 回复风格: <文本>`（单一注入缝 `_render_tone_profile()`），为空时逐字节不变；经 `SoulEngine._reply_style` 分发给对话 `LLMService` 与 `ProfileBuilder`，推荐侧由 `RecommendationEngine._reply_style` 独立接线
