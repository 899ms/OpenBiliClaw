...
| `POST /api/chat/sessions` | ✅ | 创建多会话对话（M5）。body `{"session_id?", "title?", "metadata?}`：`session_id` 缺省自动生成 `chat-<uuid>`；`title` 可留空，首条 chat 消息后服务端异步自动生成（见下）；`metadata` 是 additive JSON bag，预留 skill 绑定（M4）。同 id 幂等。返回 `ChatSessionOut`。 |
...
| `PATCH /api/chat/sessions/{session_id}` | ✅ | 改名 / 归档（M5）：body `{"title?": 非空字符串, "archived?": bool}`；空标题 422，归档默认会话 422，未知 id 404。不提供 DELETE——删除即归档。 |
...
| `GET /api/chat/approvals` | ✅ | M7 L2 审批门：列出 hard_write 待批准动作（`?status=pending|approved|rejected|executed|expired` 过滤，非法 status 422，`?limit=` 默认 50，按创建时间倒序）。返回 `{"count", "items":[ApprovalRecord]}`：每条含 `approval_id` / `tool_name` / `arguments` / `summary`（做什么）/ `reason`（为什么）/ `impact`（影响说明）/ `session` / `session_id` / `turn_id` / `status` / 时间戳 / `result` / `error`。存储为 `{data_dir}/chat_approvals.json`（免迁移）；审批未接线时 503。 |
| `POST /api/chat/approvals/{approval_id}/approve` | ✅ | 批准并**执行**一项待批准动作：状态机 `pending→approved→executed`，用登记时的原 arguments 对 `ctx.agent_tool_registry` 二次 dispatch 真写入；approve→execute→mark 由端点级 asyncio 锁串行，重复 approve 已执行记录返回 `already_executed=true` 与既有结果（幂等，不重执行）；rejected/expired 409，未知 id 404。执行后写审计台账（`profile_update_ledger`，`write_point=agent.approval.<tool>`、`gate_verdict=approved`）并往来源 turn 的 `payload.agent_events` 追加 `approval_result` 事件。响应 `{"approval", "executed", "already_executed", "ok", "result"}`；工具执行失败（`ok=false`）仍返回 200，失败细节在 `result`/`approval.error`。 |
| `POST /api/chat/approvals/{approval_id}/reject` | ✅ | 拒绝一项待批准动作（body 可带 `{"reason": "..."}`，可空）：`pending→rejected`，动作永不执行；写审计台账（`gate_verdict=rejected`）并追加 `approval_result` 回放事件。重复 reject 幂等返回现状且不重复写审计；已 approved/executed 的记录 409，未知 id 404。 |
